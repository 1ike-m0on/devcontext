...
public class RestApiExceptionHandler {
    ...
    public RestApiExceptionHandler(DevContextLlmProperties llmProperties) {
    ...
    public ResponseEntity<ApiResponse<Void>> handleApiException(ApiException exception) {
        return ResponseEntity.status(exception.status())
                .body(ApiResponse.fail(exception.errorCode(), safeMessage(exception.getMessage())));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleUnexpected(Exception exception) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiResponse.fail("INTERNAL_ERROR", safeMessage(exception.getMessage())));
    }

    private String safeMessage(String message) {
        return llmProperties.maskSecrets(message);
    }
}
